...
impl MergeProcessor {
    ...
    #[tracing::instrument(err(Debug), level = "debug", skip(self), fields(ctx.id = self.ctx.get_id().as_str()))]
...
impl Processor for MergeProcessor {
    ...
    #[tracing::instrument(err(Debug), level = "debug", name="merge_processor_execute", skip(self), fields(ctx.id = self.ctx.get_id().as_str()))]
